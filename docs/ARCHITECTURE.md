...
const RedisKeys = {
  ...
  followingFeed: (userId: string, page: number) => 
    `qotes:feed:following:${userId}:${page}`,
...
// Recurring jobs
await cronQueue.add('daily-quote', {}, {
  repeat: { pattern: '0 9 * * *' } // 9 AM daily
});

await cronQueue.add('cache-cleanup', {}, {
...
**Reaction Worker**:
```typescript
const reactionWorker = new Worker('reaction-persistence', async (job) => {
